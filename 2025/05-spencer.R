...
options(scipen = 99)
# raw <- readLines("05-example.txt")
raw <- readLines("05-input.txt")
...
# TODO this doesn't quite work on the example...
## need to turn reduce_ids to sequences, unlist, unique
map(reduce_ids, ~{max(.x) - min(.x) + 1}) %>%
  unlist %>% 
  sum

# TODO attempt #1
map(reduce_ids, ~{min(.x):max(.x)}) %>%
  unlist() %>%
  unique() %>%
  length
